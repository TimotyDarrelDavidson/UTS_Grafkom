...
export function createTrapinch(Gl, SHADER_PROGRAM, attribs) {
  ...
  const TrapinchBody  = generateTrapinchBody(0.8, 0.65, 0.9, 14, 20, [240/255,150/255,70/255]);
  const TrapinchHead  = generateTrapinchHead(1.0, 0.9, 1.2, 50, 30, [240/255,150/255,70/255]);
  const TrapinchEyes  = generateTrapinchEyes2(0.05, 0.20, 0.20, 10, 12, [0,0,0]);    // black
  const TrapinchPupil = generateTrapinchEyes2(0.01, 0.08, 0.06,  8, 10,  [1,1,1]);  // white
  const TrapinchNeck  = generateTrapinchNeck(0.325, 0.2, 0.6, 1.2, 8, 16, [0.90,0.53,0.22]);
  const TrapinchCracks = generateTrapinchHeadCracks(7, 30, [40/255,25/255,10/255]);

  const TrapinchLeg   = generateTrapinchLegs(0.22, 25, 10, [0.90,0.53,0.22]);
  const TrapinchTip   = generateTrapinchLegTip(0.22, 0.05, 0.22, 10, 12, [220/255,140/255,80/255]);

  // ─────────────── Objects ───────────────
  const Trapinch = new MyObject(Gl, SHADER_PROGRAM, _position, _color, _Mmatrix,
    TrapinchBody.vertices, TrapinchBody.faces);

  const TNeck  = new MyObject(Gl, SHADER_PROGRAM, _position, _color, _Mmatrix,
    TrapinchNeck.vertices, TrapinchNeck.faces);

  const THead  = new MyObject(Gl, SHADER_PROGRAM, _position, _color, _Mmatrix,
    TrapinchHead.vertices, TrapinchHead.faces);

  const TCracks = new MyObject(Gl, SHADER_PROGRAM, _position, _color, _Mmatrix,
    TrapinchCracks.vertices, TrapinchCracks.faces);

  const TREyes = new MyObject(Gl, SHADER_PROGRAM, _position, _color, _Mmatrix,
    TrapinchEyes.vertices, TrapinchEyes.faces);
  const TLEyes = new MyObject(Gl, SHADER_PROGRAM, _position, _color, _Mmatrix,
    TrapinchEyes.vertices, TrapinchEyes.faces);

  const TRPupil = new MyObject(Gl, SHADER_PROGRAM, _position, _color, _Mmatrix,
    TrapinchPupil.vertices, TrapinchPupil.faces);
  const TLPupil = new MyObject(Gl, SHADER_PROGRAM, _position, _color, _Mmatrix,
    TrapinchPupil.vertices, TrapinchPupil.faces);

  // Legs
  const TRFLeg = new MyObject(Gl, SHADER_PROGRAM, _position, _color, _Mmatrix, TrapinchLeg.vertices, TrapinchLeg.faces);
  const TRBLeg = new MyObject(Gl, SHADER_PROGRAM, _position, _color, _Mmatrix, TrapinchLeg.vertices, TrapinchLeg.faces);
  const TLFLeg = new MyObject(Gl, SHADER_PROGRAM, _position, _color, _Mmatrix, TrapinchLeg.vertices, TrapinchLeg.faces);
  const TLBLeg = new MyObject(Gl, SHADER_PROGRAM, _position, _color, _Mmatrix, TrapinchLeg.vertices, TrapinchLeg.faces);

  // Leg tips
  const TRFTip = new MyObject(Gl, SHADER_PROGRAM, _position, _color, _Mmatrix, TrapinchTip.vertices, TrapinchTip.faces);
  const TRBTip = new MyObject(Gl, SHADER_PROGRAM, _position, _color, _Mmatrix, TrapinchTip.vertices, TrapinchTip.faces);
  const TLFTip = new MyObject(Gl, SHADER_PROGRAM, _position, _color, _Mmatrix, TrapinchTip.vertices, TrapinchTip.faces);
  const TLBTip = new MyObject(Gl, SHADER_PROGRAM, _position, _color, _Mmatrix, TrapinchTip.vertices, TrapinchTip.faces);
  ...
  LIBS.translateZ(TRFLeg.MOVE_MATRIX,  0.40);
  LIBS.translateY(TRFLeg.MOVE_MATRIX, -0.20);

  LIBS.translateX(TRBLeg.MOVE_MATRIX,  0.55);
  LIBS.translateZ(TRBLeg.MOVE_MATRIX, -0.30);
  LIBS.translateY(TRBLeg.MOVE_MATRIX, -0.20);

  LIBS.translateX(TLFLeg.MOVE_MATRIX, -0.55);
  LIBS.translateZ(TLFLeg.MOVE_MATRIX,  0.40);
  LIBS.translateY(TLFLeg.MOVE_MATRIX, -0.20);
  LIBS.rotateY(TLFLeg.MOVE_MATRIX, -Math.PI / 2);

  LIBS.translateX(TLBLeg.MOVE_MATRIX, -0.55);
  LIBS.translateZ(TLBLeg.MOVE_MATRIX, -0.30);
  LIBS.translateY(TLBLeg.MOVE_MATRIX, -0.20);
  ...
  function animate(t, orbit = { theta: 0, phi: 0, camDist: 10 }) {
    ...
    // Orbit controls
    let tmp = LIBS.get_I4();
    LIBS.rotateY(tmp, orbit.theta); Trapinch.MOVE_MATRIX = LIBS.multiply(Trapinch.MOVE_MATRIX, tmp);
    tmp = LIBS.get_I4();
    LIBS.rotateX(tmp, orbit.phi);   Trapinch.MOVE_MATRIX = LIBS.multiply(Trapinch.MOVE_MATRIX, tmp);
    ...
    tmp = LIBS.get_I4(); LIBS.translateY(tmp, -0.2 + bob);
    ...
    LIBS.set_I4(TRPupil.MOVE_MATRIX); LIBS.translateX(TRPupil.MOVE_MATRIX, 0.08);
    LIBS.set_I4(TLPupil.MOVE_MATRIX); LIBS.translateX(TLPupil.MOVE_MATRIX, -0.08);
    ...
    LIBS.translateX(TRFLeg.MOVE_MATRIX, 0.55);
    LIBS.translateZ(TRFLeg.MOVE_MATRIX, 0.40 + swing);
    LIBS.translateY(TRFLeg.MOVE_MATRIX, -0.20 + lift);
    LIBS.rotateX(TRFLeg.MOVE_MATRIX, swing * 0.5);

    // RB (opposite)
    LIBS.set_I4(TRBLeg.MOVE_MATRIX);
    LIBS.translateX(TRBLeg.MOVE_MATRIX, 0.55);
    LIBS.translateZ(TRBLeg.MOVE_MATRIX, -0.30 - swing);
    LIBS.translateY(TRBLeg.MOVE_MATRIX, -0.20 + Math.max(0, -phase) * 0.15);
    LIBS.rotateX(TRBLeg.MOVE_MATRIX, -swing * 0.5);

    // LF
    LIBS.set_I4(TLFLeg.MOVE_MATRIX);
    LIBS.translateX(TLFLeg.MOVE_MATRIX, -0.55);
    LIBS.translateZ(TLFLeg.MOVE_MATRIX, 0.40 - swing);
    LIBS.translateY(TLFLeg.MOVE_MATRIX, -0.20 + Math.max(0, -phase) * 0.15);
    LIBS.rotateY(TLFLeg.MOVE_MATRIX, -Math.PI / 2);
    ...
    LIBS.translateX(TLBLeg.MOVE_MATRIX, -0.55);
    LIBS.translateZ(TLBLeg.MOVE_MATRIX, -0.30 + swing);
    LIBS.translateY(TLBLeg.MOVE_MATRIX, -0.20 + lift);
    LIBS.rotateY(TLBLeg.MOVE_MATRIX, -Math.PI / 2);
